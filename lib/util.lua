...
function math.getNorm(dx,dy)
	return ((dx)^2+(dy)^2)^0.5
end
function math.getDistance(x1,y1,x2,y2)
	return math.getNorm(x1-x2,y1-y2)
end
function math.axisRot(dx,dy,rot)
	return math.cos(rot)*dx-math.sin(rot)*dy,math.cos(rot)*dy+math.sin(rot)*dx
end
...
function math.getNormVertor(dx,dy)
	local rot=math.atan2(dx,dy)
	return dx*math.sin(dx),dy*math.cos(dy)
end
...
function physics.newBox(x,y,w,h,t,density)
...
function physics.newCircle(x,y,radius,t,density)
	local body = love.physics.newBody(physics.world, x, y, t or "dynamic")
...
function physics.newPolygon(x,y,verts,t,density)
	local body = love.physics.newBody(physics.world, x, y, t or "dynamic")
...
function physics.anyPolygon(x,y,verts,t,density)
	local tri=love.math.triangulate(verts)
	local body = love.physics.newBody(physics.world, x, y, t or "dynamic")
	for i,v in ipairs(tri) do
		local shape = love.physics.newPolygonShape(v)
		local fixture = love.physics.newFixture(body, shape, density)
	end
	return body
end
...
function uid(prefix)
	local template =prefix .. '-xxxx-yxxx-xxxxxxxxxxxx'
