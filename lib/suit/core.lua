...
function suit:anyHovered()
	return self.hovered ~= nil,self.hovered
end
...
function suit:mouseDragRelease(id)
    if not self.mouse_button_down and not self:isActive(id) and self:isHovered(id) and self.active~=id then
		self.dragFrom = self.active
        self.dragTo = id
		return self.dragFrom
	end
	return false 
end

function suit:mouseDragDown(id)
    if self.mouse_button_down and self:isActive(id) and self:isHovered(id) then
		return true
	end
	return false 
end
